(() => {
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) new swiper_core_Swiper(".swiper", {
            ...
            modules: [ Navigation, Pagination, EffectFade, EffectCube, EffectFlip, EffectCoverflow, EffectCards, EffectCreative ]
        });
        if (document.querySelector(".vSwiper")) new swiper_core_Swiper(".vSwiper", {
            direction: "vertical",
            grabCursor: true,
            slidesPerView: 1,
            spaceBetween: 0,
        ...
        if (document.querySelector(".val-swiper")) new swiper_core_Swiper(".val-swiper", {
            grabCursor: true,
            slidesPerView: 3,
            spaceBetween: 60,
            speed: 500,
    ...
    class HeaderTopScroll {
        ...
        scrollHeaderTopHandler(type) {
            ...
            document.addEventListener("scroll", (() => {
                ...
                if (scroll > header.clientHeight / 2) header.classList.add(`${classEl}`); else header.classList.remove(`${classEl}`);
            }));
        }
    }
    const menuBurger_isNaN = value => value === null || value === void 0;
    const activeMenuBurger = (menu, activeClass, option = {}) => {
        const {nameClassBlockedScroll, btnOpenSelector, btnCloseSelector} = option;
        function openMenu() {
            if (menuBurger_isNaN(menu)) {
                console.error(`Елемент не передан в параметр`);
                return;
            }
            menu.classList.add(activeClass);
            document.body.classList.add(nameClassBlockedScroll || "menu__blocked-scroll-content");
        }
        function closeMenu() {
            if (menuBurger_isNaN(menu)) {
                console.error(`Елемент не передан в параметр`);
            ...
            document.body.classList.remove(nameClassBlockedScroll || "menu__blocked-scroll-content");
        }
        function toggleMenu() {
            if (menuBurger_isNaN(menu)) {
                console.error(`Елемент не передан в параметр`);
            ...
            document.body.classList.toggle(nameClassBlockedScroll || "menu__blocked-scroll-content");
        }
        function openBtnActionMenu() {
            if (menuBurger_isNaN(btnOpenSelector)) {
        ...
        function closeBtnActionMenu() {
            if (menuBurger_isNaN(btnCloseSelector)) {
                console.error(`Елемент не найден`);
                return;
            }
            document.getElementById(btnCloseSelector).addEventListener("click", (() => {
                if (menu.classList.contains(activeClass)) this.closeMenu();
            }));
        }
        function init() {
            ...
            const handleClickOutside = e => {
                if (menu && !menu.contains(e.target) && menu.classList.contains(activeClass)) this.closeMenu();
            };
